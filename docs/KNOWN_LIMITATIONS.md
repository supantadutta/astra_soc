...
| **Live connector writes** | Adapters implement health + read + mock write. The real `execute_action` for each vendor raises `NotImplementedError` rather than guessing an API. | Implement each vendor's action API; the response gateway already reports the true outcome and never fabricates success. |
